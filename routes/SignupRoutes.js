...
router.post('/signup', async (req, res) => {
    ...
    try {
        ...
        if (existingUser) {
            res.status(400).json({ message: 'User already exists. Please choose a different email.' });
            return;
            ...
            transporter.sendMail(mailOptions, (error, info) => {
                if (error) {
                    ...
                    res.status(200).json({ message: 'Please check your email to confirm your account. Confirm in less than 10 minutes!' });
                }
            });
        }
    } catch (error) {
